...
const AGENT_DEFINITIONS: Record<AgentRole, AgentDefinition> = {
  orchestrator: {
    ...
    systemPrompt: [
      ...
      '5. 사용자 요청 산출물을 answer 또는 implementation으로 분류한 deliverableType',
      '',
      ...
      '5. deliverableType이 implementation이면 실제 workspace 변경 파일, 실행 명령, 검증 결과가 모두 확인되기 전에는 complete로 판단하지 않는다.',
      '',
      ...
      '- deliverableType은 설명/분석/계획이면 "answer", 실제 파일 생성/수정/개발/구현이면 "implementation"을 사용한다.',
  ...
  engineer: {
    ...
    systemPrompt: [
      ...
      'deliverableType이 implementation이면 설명으로 끝내지 말고 지정된 workspace에 실제 파일을 생성/수정한다.',
      'implementation 결과에는 changedFiles, commandsRun, testResults, remainingRisks를 반드시 포함한다.',
