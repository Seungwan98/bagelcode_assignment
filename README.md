...
- 채팅방의 선택 agent, Logs/실행 요약 열림 상태는 run별로 브라우저에 복원됩니다.
...
- “앱 개발/파일 수정” 같은 실제 구현 요청은 `.agentboard/workspaces/<runId>/` 산출물과 검증 증거가 있어야 완료 처리됩니다.
...
6. Agents가 답변을 생성하는 동안에도 composer에서 추가 지시를 보낼 수 있고 현재 작업 indicator와 `취소` 버튼이 표시되는지 확인합니다.
